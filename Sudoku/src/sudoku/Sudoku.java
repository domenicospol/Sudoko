...
public class Sudoku {
    ...
    private void generatePuzzle()  {
        ...
        for (int i = 0; i < 9; i++)
            for (int j = 0; j < 9; j++) {
                ...
                //Step 2: remove numbers that have been taken by that row
                pool = updatePool(pool, puzzle[i]);
                
                //Step 3: remove numbers that have been taken by that col
                pool = updatePool(pool, extractOneColumn(j));
                    
                //Step 4: remove numbers that have been taken in the square
                pool = updatePool(pool, extractOneSquare(i, j));
                
                //Step 5: Randomly select a number from the pool
                if (!pool.isEmpty())
    ...
    private ArrayList<Integer> initilizePool() {
